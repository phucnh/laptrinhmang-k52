...
void CMimeEnvironment::RegisterFieldCoder(const char* pszFieldName, FIELD_CODER_FACTORY pfnCreateObject)
{
...
int CMimeEncodedWord::Decode(unsigned char* pbOutput, int nMaxSize)
{
	...
	while (pbData < pbEnd)
	{
		...
		if ( nMaxSize > 2 && pbData[0] == '=' && pbData[1] == '?')	
		{
...
int CMimeEncodedWord::BEncode(unsigned char* pbOutput, int nMaxSize) const
{
	...
	int nBlockSize = MAX_ENCODEDWORD_LEN - nCharsetLen - 7;	
